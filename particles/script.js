...
function updateBoid(boid, neighbors) {
    ...
    for (const otherBoid of neighbors) {
        // Skip if either boid is fading out
        if (boid === otherBoid || boid.isFadingOut || otherBoid.isFadingOut) continue;
    ...
    // Only update grid position if not fading out
    if (!boid.isFadingOut) {
        state.updateBoidInGrid(boid);
    }
    ...
    for (let i = state.simulation.boids.length - 1; i >= 0; i--) {
        ...
        if (!boid.isFadingOut) {
            ...
            if (updateBoid(boid, neighbors)) {
                ...
                // Remove from grid immediately when starting fade out
